...
# Array para almacenar los datos obtenidos por RK4
x = np.zeros((len(t), 2))
...
for i in range(len(t)-1):
    ...
    k2 = np.array([yd1(t[i] + 0.5*dT, x[i, :] + k1*0.5*dT), yd2(t[i] + 0.5*dT, x[i, :] + k1*0.5*dT)])
    k3 = np.array([yd1(t[i] + 0.5*dT, x[i, :] + 0.5*k2*dT), yd2(t[i] + 0.5*dT, x[i, :] + 0.5*k2*dT)])
    k4 = np.array([yd1(t[i] + dT, x[i, :] + k3*dT), yd2(t[i] + dT, x[i, :] + k3*dT)])
    x[i+1, :] = x[i, :] + 1/6*(k1 + 2*k2 + 2*k3 + k4)*dT

# Creo tabla apartir de los datos obtenidos por RK4
...
writer = pd.ExcelWriter('Datos_RK4.xlsx', engine='openpyxl')
...
def graph1():
    ...
    ax1 = plt.gca()
    ax1.set_title('Aproximación númerica de x[0] vs t por método RK4')
    ax1.set_xlabel('$X$')
    ax1.set_ylabel('$Y$')
    ax1.axes.grid(True)
    ax1.legend()
...
def graph2():
    ...
    ax2 = plt.gca()
    ax2.set_title('Aproximación númerica de x[1] vs t por método RK4')
    ax2.set_xlabel('$X$')
    ax2.set_ylabel('$Y$')
    ax2.axes.grid(True)
    ax2.legend()
...
def graph3():
    ...
    plt.plot(tg, sol[:, 1], label='real')
    ax3 = plt.gca()
    ax3.set_title('Aproximación númerica númerica de x[0] & x[1] vs t por método RK4')
    ax3.set_xlabel('$X$')
    ax3.set_ylabel('$Y$')
...
def graph5():
    plt.figure(5)
    plt.plot(t, sol[::100, 0] - x[:, 0], label='Error absoluto de x[0]')
    ax1 = plt.gca()
    ax1.set_title('Error absoluto de x[0] vs t por método RK4')
    ax1.set_xlabel('$X$')
    ax1.set_ylabel('$Y$')
    ax1.axes.grid(True)
    ax1.legend()
...
def graph6():
    plt.figure(6)
    plt.plot(t, sol[::100, 1] - x[:, 1], label='Error absoluto de x[1]')
    ax1 = plt.gca()
    ax1.set_title('Error absoluto de x[1] vs t por método RK4')
    ax1.set_xlabel('$X$')
    ax1.set_ylabel('$Y$')
    ax1.axes.grid(True)
    ax1.legend()
...
# root = tk.Tk()
# root.title('Hola mundo! esta es mi solución de la ED con RK4')
