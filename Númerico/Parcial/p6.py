...
x = x_0
k = 0
while k < n_max and x < x_f:
    sx = np.append(sx, x)
    sy = np.concatenate((sy, y_k5), axis=0)
    if h < h_min:
        h = h_min
    if h > h_max:
        h = h_max
    ...
    k4 = f(x + h, y - k2 + 2*k3)
    k5 = f(x + h, y + 1/3*k1 + 1/6*k2 + 1/6*k3 + 1/3*k4)
    y_k4 = y + 1/6*k1 + 4/6*k3 + 1/6*k4
    y_k5 = y + 3/7*k1 + 3/7*k2 - 2/7*k3 + 1/7*k4 + 2/7*k5
    e = np.abs(y_k4[0] - y_k5[0])
